...
2. Copy the AMI to your account
   ...
   **Restore image**
    ...
    ```shell
    aws ec2 create-restore-image-task --object-key ami-0ec6fdfb365e5fc00.bin --bucket udacity-srend --name "udacity-gpm"
    ```
    ...
    - Take note of that AMI ID the script just output. Copy the AMI to `us-east-2` and `us-west-1`:
        - `aws ec2 copy-image --source-image-id ami-04426d67a5a5050c8 --source-region us-east-1 --region us-east-2 --name "udacity-gpm"`
        - `aws ec2 copy-image --source-image-id ami-04426d67a5a5050c8 --source-region us-east-1 --region us-west-1 --name "udacity-gpm"`
